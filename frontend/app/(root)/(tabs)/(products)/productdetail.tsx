  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.text}>
          Usage Instructions: {product.specifications.usageInstructions}
        </Text>
        {product.specifications.expiryDate && (
          <Text style={styles.text}>
            Expiry Date:{" "}
            {new Date(product.specifications.expiryDate).toLocaleDateString()}
          </Text>
        )}
        {product.specifications.cropSuitability &&
          product.specifications.cropSuitability.length > 0 && (
            <Text style={styles.text}>
              Crop Suitability:{" "}
              {product.specifications.cropSuitability.join(", ")}
            </Text>
          )}
        {product.specifications.soilType &&
          product.specifications.soilType.length > 0 && (
            <Text style={styles.text}>
              Soil Type: {product.specifications.soilType.join(", ")}
            </Text>
          )}
        <Text style={styles.text}>
          Organic: {product.specifications.organic ? "Yes" : "No"}
        </Text>

        <Text style={styles.sectionTitle}>Shipping Details</Text>
        <Text style={styles.text}>Weight: {product.shipping.weight} kg</Text>
        <Text style={styles.text}>
          Dimensions: {product.shipping.dimensions.length}cm x{" "}
          {product.shipping.dimensions.width}cm x{" "}
          {product.shipping.dimensions.height}cm
        </Text>
        <Text style={styles.text}>
          Delivery Time: {product.shipping.deliveryTimeInDays} days
        </Text>

        <Text style={styles.sectionTitle}>Stock</Text>
        <Text style={styles.text}>
          Available Quantity: {product.stock.quantity}
        </Text>
        <Text style={styles.text}>
          Low Stock Threshold: {product.stock.lowStockThreshold}
        </Text>
