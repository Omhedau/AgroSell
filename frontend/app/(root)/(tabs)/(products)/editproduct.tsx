...
const EditProduct = () => {
  ...
  useEffect(() => {
    if (parsedProduct) {
      setFormData({
        ...
        price: {
          mrp: parsedProduct.price.mrp.toString(),
          sellingPrice: parsedProduct.price.sellingPrice.toString(),
          discountPercentage: parsedProduct.price.discountPercentage,
        },
        stock: {
          quantity: parsedProduct.stock.quantity.toString(),
          lowStockThreshold: parsedProduct.stock.lowStockThreshold.toString(),
        },
        specifications: {
          weight: parsedProduct.specifications.weight,
          composition: parsedProduct.specifications.composition,
          usageInstructions: parsedProduct.specifications.usageInstructions,
          expiryDate: parsedProduct.specifications.expiryDate.$date,
          cropSuitability: parsedProduct.specifications.cropSuitability.join(", "),
          soilType: parsedProduct.specifications.soilType,
          organic: parsedProduct.specifications.organic,
        },
        shipping: {
          weight: parsedProduct.shipping.weight.toString(),
          dimensions: {
            length: parsedProduct.shipping.dimensions.length.toString(),
            width: parsedProduct.shipping.dimensions.width.toString(),
            height: parsedProduct.shipping.dimensions.height.toString(),
          },
          deliveryTimeInDays: parsedProduct.shipping.deliveryTimeInDays.toString(),
        },
        tags: parsedProduct.tags.join(", "),
        returnPolicy: parsedProduct.returnPolicy,
      });
    }
  },[] ); // Run only when parsedProduct changes
  ...
  const updateProduct = async (formData: any, fileType: string) => {
    try {
      ...
      if (formData.images?.length > 0 && formData.images[0].startsWith("file://")) {
        try {
      ...
      const productData = {
        ...
        images: uploadedImageUrl ? [uploadedImageUrl] : formData.images, // Use the uploaded image URL if available
        price: {
          mrp: mrp,
      ...
      const response = await axios.put(
        `${constants.base_url}/api/products/${parsedProduct._id}`,
        productData,
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      updateProduct(formData, fileType);
    } else {
